...
do_action('customize_save_after', 'wpresidence_customizer_savesettings', 10);{
    if(has_site_icon()){
        $values= array();
...
function wpestate_pagination_ajax_directory($pages = '', $range = 2,$paged,$where,$order){  
    ...
    if(1 != $pages){
        ...
        print "<li class=\"roundleft\"><a href=\"#\" data-future='".esc_attr($prev_page)."'><i class=\"fa fa-angle-left\"></i></a></li>";
        ...
        if ( ($paged +1) > $pages){
            ...
            print "<li class=\"roundright\"><a href='#' data-future='".esc_attr($paged)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
        }else{
            $next_page= get_pagenum_link($paged + 1);
            $next_page = add_query_arg( 'order', $order,$next_page );
            print "<li class=\"roundright\"><a href='#' data-future='".esc_attr($paged+1)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
        }
...
function wpestate_return_all_fields($is_mandatory=0){
    ...
    $wpestate_submission_page_fields     =   ( get_option('wp_estate_submission_page_fields','') );
...
function wpestate_walkscore_details($post_id) {
    ...
    if(isset($walkscore->walkscore)){
        ...
        if(isset($tranzit_score)){
            print '<div class="walk_details"><img src="https://cdn.walk.sc/images/transit-score-logo.png" alt="'.esc_html__('walkscore','wpresidence').'">';
            print '<span>'.$tranzit_score->transit_score.' / '. $tranzit_score->description.'</span>';
            print '<span class="" >'.$tranzit_score->summary.': </a>';
            print '<a href="'.esc_url($tranzit_score->ws_link).'" target="_blank">'.esc_html__('more details here','wpresidence').'</a> </span></div>';   
        }
    }
} 
endif;
...
function wpestate_pagination($pages = '', $range = 2){  
    ...
    if(1 != $pages){
        ...
        print "<li class=\"roundleft\"><a href='".get_pagenum_link($paged - 1)."'><i class=\"fa fa-angle-left\"></i></a></li>";
        ...
        print "<li class=\"roundright\"><a href='".$prev_page."'><i class=\"fa fa-angle-right\"></i></a><li></ul>";
    }
}
endif; // end   wpestate_pagination  
...
function wpestate_pagination_agent($pages = '', $range = 2){  
    ...
    if(1 != $pages){ 
         ...
         print '<ul class="pagination pagination_nojax">';
         print "<li class=\"roundleft\"><a href='".$prev_pagex."'><i class=\"fa fa-angle-left\"></i></a></li>";
...
{  
    $showitems = ($range * 2)+1;  
     ...
     if(1 != $pages)
     {
        ...
        print "<li class=\"roundleft\"><a href='".$prev_link."' data-future='".esc_attr($prev_page)."'><i class=\"fa fa-angle-left\"></i></a></li>";
        ...
        if ( ($paged +1) > $pages){
            $next_page= get_pagenum_link($paged );
            $next_page = add_query_arg( 'order', $order,$next_page );
            print "<li class=\"roundright\"><a href='".esc_url($next_page)."' data-future='".esc_attr($paged)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
        }else{
            $next_page= get_pagenum_link($paged + 1);
            $next_page = add_query_arg( 'order', $order,$next_page );
            print "<li class=\"roundright\"><a href='".esc_url($next_page)."' data-future='".esc_attr($paged+1)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
        }
...
{  
    $showitems = ($range * 2)+1;  
     ...
     if(1 != $pages)
     {
         ...
         print "<li class=\"roundleft\"><a href='".esc_url(get_pagenum_link($paged - 1))."' data-future='".esc_attr($prev_page)."'><i class=\"fa fa-angle-left\"></i></a></li>";
         ...
         if ( ($paged +1) > $pages){
            $prev_page= get_pagenum_link($paged );
             print "<li class=\"roundright\"><a href='".esc_url($prev_page)."' data-future='".esc_attr($paged)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
         }else{
             $prev_page= get_pagenum_link($paged + 1);
             print "<li class=\"roundright\"><a href='".esc_url($prev_page)."' data-future='".esc_attr($paged+1)."'><i class=\"fa fa-angle-right\"></i></a><li>"; 
         }
        ...
        <div data-toggle="dropdown" id="'.sanitize_key( $appendix.$toogle_id ).'" class="'.$extraclass.'" xx '.$values1.' '.$values.' data-value="'.( esc_attr( $current_val1) ).'">';
            ...
            if (  $get_var=='filter_search_type' || $get_var=='filter_search_action' || $get_var=='advanced_city' || $get_var=='advanced_area' || $get_var=='advanced_conty' || $get_var=='advanced_contystate'){
                if($show_val=='All'){
                    ...
                    if($get_var=='filter_search_type'){
                        $return_string.=  esc_html__('All Types','wpresidence');
                    }else if($get_var=='filter_search_action'){
                        $return_string.= esc_html__('All Actions','wpresidence');
                    }else if($get_var=='advanced_city' ){
                        $return_string.= esc_html__('All Cities','wpresidence');
                    }else if($get_var=='advanced_area'){
                        $return_string.=esc_html__('All Areas','wpresidence');
                    }else if($get_var=='advanced_conty'){
                        $return_string.= esc_html__('All Actions','wpresidence');
                    }else if($get_var=='advanced_contystate'){
                        $return_string.= esc_html__('All Counties/States','wpresidence');
                    }
    ...
    function  wpestate_show_search_field_with_tabs($position,$search_field,$action_select_list,$categ_select_list,$select_city_list,$select_area_list,$key,$select_county_state_list,$use_name,$term_id){
        ...
        if($search_field=='none'){
            ...
            if(isset($_GET['filter_search_action'][0]) && $_GET['filter_search_action'][0]!='' && $_GET['filter_search_action'][0]!='all'){
                ...
                $adv_actions_value  =   esc_html__('All Actions','wpresidence');
            ...
            if(isset($_GET['advanced_city']) && $_GET['advanced_city']!='' && $_GET['advanced_city']!='all'){
                ...
                $advanced_city_value    =   esc_html__('All Cities','wpresidence');
            ...
            if(isset($_GET['advanced_area']) && $_GET['advanced_area']!=''  && $_GET['advanced_area']!='all'){
                ...
                $advanced_area_value    =   esc_html__('All Areas','wpresidence');
            ...
            if(isset($_GET['advanced_contystate']) && $_GET['advanced_contystate']!='' && $_GET['advanced_contystate']!='all' ){
                ...
                $advanced_county_value  = esc_html__('All Counties/States','wpresidence');
...
if( !function_exists('wpestate_get_city_select_list') ): 
    function wpestate_get_city_select_list($args){
        ...
        if($categ_select_list===false){
            ...
            $categ_select_list   =    '<li role="presentation" data-value="all" data-value2="all">'. esc_html__('All Cities','wpresidence').'</li>';
            $taxonomy           =   'property_city';
            $categories     =   get_terms($taxonomy,$args);
            if(is_array($categories)){
                foreach ($categories as $categ) {
                    ...
                    $categ_select_list  .=  '<li role="presentation" data-value="'.esc_attr($categ->slug).'" data-value2="'.esc_attr($slug).'" data-parentcounty="'.$slug_county.'">'. ucwords ( urldecode( $categ->name ) ).' ('.$counter.')'.'</li>';
                    if(isset($received['html'])){
                        $categ_select_list     .=   $received['html'];  
                    }
...
function wpestate_get_county_state_select_list($args){
    ...
    if($categ_select_list===false){
        $categ_select_list  =   '<li role="presentation" data-value="all" data-value2="all">'.esc_html__('All Counties/States','wpresidence').'</li>';
        $taxonomy           =   'property_county_state';
        $categories         =   get_terms($taxonomy,$args);
...
function wpestate_get_area_select_list($args){
    ...
    if($categ_select_list===false){
        ...
        $categ_select_list  =   '<li role="presentation" data-value="all">'.esc_html__('All Areas','wpresidence').'</li>';
        $taxonomy           =   'property_area';
        $categories         =   get_terms($taxonomy,$args);
...
function wpestate_show_stripe_form_per_listing($stripe_class,$post_id,$price_submission,$price_featured_submission){
    ...
    <form action="'.$processor_link.'" method="post" class="stripe_form_simple">
        <div class="stripe_simple">
            ...
            data-amount="'.esc_attr($price_submission).'" 
            data-email="'.esc_attr($user_email).'"
            data-zip-code="true"
            data-locale="auto"
            data-currency="'.esc_attr($submission_curency_status).'"
            data-label="'.esc_attr__('Pay with Credit Card','wpresidence').'"
            data-description="'.esc_attr__('Submission Payment','wpresidence').'">
            </script>
        </div>
    ...
    <form action="'.$processor_link.'" method="post" class="stripe_form_featured">
        <div class="stripe_simple">
            ...
            class="stripe-button"
            data-key="'. esc_attr($stripe_publishable_key).'"
            data-amount="'.esc_attr($price_submission_total).'" 
            data-email="'.esc_attr($user_email).'"
            data-zip-code="true"
            data-locale="auto"
            data-currency="'.esc_attr($submission_curency_status).'"
            data-label="'.esc_attr__('Pay with Credit Card','wpresidence').'"
            data-description="'.esc_attr__('Submission & Featured Payment','wpresidence').'">
            </script>
        </div>
        <input type="hidden" id="propid" name="propid" value="'.esc_attr($post_id).'">
        <input type="hidden" id="submission_pay" name="submission_pay" value="1">
        <input type="hidden" id="featured_pay" name="featured_pay" value="1">
        <input type="hidden" name="userID" value="'.esc_attr($userID).'">
        <input type="hidden" id="pay_ammout" name="pay_ammout" value="'.esc_attr($price_submission_total).'">';
        wp_nonce_field( 'wpestate_stripe_payments', 'wpestate_stripe_payments_nonce' );
    print'
    </form>
    </div>';
...
function    wpestate_show_stripe_form_upgrade($stripe_class,$post_id,$price_submission,$price_featured_submission){
    ...
    <form action="'.esc_url($processor_link).'" method="post" >
    <div class="stripe_simple upgrade_stripe">
        <script src="https://checkout.stripe.com/checkout.js" 
        class="stripe-button"
        data-key="'. esc_attr($stripe_publishable_key).'"
        data-amount="'.esc_attr($price_featured_submission).'" 
        data-zip-code="true"
        data-locale="auto"
        data-email="'.esc_attr($user_email).'"
        data-currency="'.esc_attr($submission_curency_status).'"
        data-panel-label="'.esc_attr__('Upgrade to Featured','wpresidence').'"
        data-label="'.esc_attr__('Upgrade to Featured','wpresidence').'"
        data-description="'.esc_attr__(' Featured Payment','wpresidence').'">
    ...
    <input type="hidden" name="userID" value="'.esc_attr($userID).'">
    <input type="hidden" id="pay_ammout" name="pay_ammout" value="'.esc_attr($price_featured_submission).'">';
    wp_nonce_field( 'wpestate_stripe_payments', 'wpestate_stripe_payments_nonce' );
    print'
    </form>
    </div>';
