    ...
    <div class="col-md-12 dasboard-prop-listing">
        ...
        <div class="prop-info">
            ...
            <div class="info-container">
                ...
                <?php if ($no_views>0){ ?>
                <a  data-original-title="<?php esc_attr_e('Views Stats','wpresidence');?>"   class="dashboad-tooltip show_stats"  data-listingid="<?php echo intval($post_id);?>" href="#"><i class="fa fa-eye-slash "></i></a>
                <?php }else{ ?>
                    <a  data-original-title="<?php esc_attr_e('Item has 0 views','wpresidence');?>"   class="dashboad-tooltip show_statsx"  data-listingid="<?php echo intval($post_id);?>" href="#"><i class="fa fa-eye-slash "></i></a>
                <?php }?>
                <?php
                    if( $post_status == 'expired' ){ 
                       print'<a data-original-title="'.esc_attr__('Resend for approval','wpresidence').'" class="dashboad-tooltip resend_pending" data-listingid="'.intval($post_id).'"><i class="fa fa-upload "></i></a>';   
                    }
                ?>    

                <?php
                   if( $post_status == 'publish' ){ 
                       print ' <span  data-original-title="'.esc_attr__('Disable Listing','wpresidence').'" class="dashboad-tooltip disable_listing disabledx" data-postid="'.intval($post_id).'" ><i class="fa fa-pause"></i></span>';
                   }else if($post_status=='disabled') {
                       print ' <span  data-original-title="'.esc_attr__('Enable Listing','wpresidence').'" class="dashboad-tooltip disable_listing" data-postid="'.intval($post_id).'" ><i class="fa fa-play"></i></span>';
                ...
                if($paid_submission_status=='membership'){
                    ...
                    if ( intval(get_post_meta($post_id, 'prop_featured', true))==1){
                        ...
                        print ' <span  data-original-title="'.esc_attr__('Set as featured,  *Listings set as featured are subtracted from your package','wpresidence').'" class="dashboad-tooltip make_featured" data-postid="'.intval($post_id).'" ><i class="fa fa-star favprop"></i></span>';
                    }
                }
                        ...
                        if($paid_submission_status=='per listing'){
                            ...
                            if($pay_status!='paid' ){
                                print' 
                                    ...
                                    '.esc_html__('Submission Fee','wpresidence').': <span class="submit-price submit-price-no">'.esc_html($price_submission).'</span><span class="submit-price"> '.esc_html($wpestate_currency).'</span></br>
                                    <input type="checkbox" class="extra_featured" name="extra_featured" style="display:block;" value="1" >
                                    '.esc_html__('Featured Fee','wpresidence').': <span class="submit-price submit-price-featured">'.esc_html($price_featured_submission).'</span><span class="submit-price"> '.esc_html($wpestate_currency).'</span> </br>
                                    '.esc_html__('Total Fee','wpresidence').': <span class="submit-price submit-price-total">'.esc_html($price_submission).'</span> <span class="submit-price">'.esc_html($wpestate_currency).'</span>  </br> ';
                                    print  '</div>'; 
                                    $stripe_class='';
