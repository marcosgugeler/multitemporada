...
if ($post_attachments || has_post_thumbnail() || get_post_meta($post->ID, 'embed_video_id', true)) {  ?>   
    <div id="carousel-listing" class="carousel slide post-carusel carouselvertical" data-ride="carousel" data-interval="false">
        <?php 
        if($prop_stat!='normal'){
            print '<div class="slider-property-status verticalstatus ribbon-wrapper-'.esc_attr($ribbon_class).' '.esc_attr($ribbon_class).'">'.esc_html($prop_stat). '</div>';
        }
        ?>
        ...
        if($video_id!=''){
            ...
            $indicators.='<li data-target="#carousel-listing"  data-video_data="'.esc_attr($video_type).'" data-video_id="'.esc_attr($video_id).'"  data-slide-to="0" class="active video_thumb_force">
                         <img src= "'.wpestate_get_video_thumb($post->ID).'" alt="'.esc_html__('video thumb','wpresidence').'" class="img-responsive lightbox_trigger"/>
        ...
        if( has_post_thumbnail() ){
            ...
            $indicators.= '<li data-target="#carousel-listing" data-slide-to="'.($counter-1).'" class="'.esc_attr($active).'">
                                <img  src="'.esc_url($preview[0]).'"  alt="'.esc_html__('image','wpresidence').'" />
                           </li>';

            $round_indicators   .=  ' <li data-target="#carousel-listing" data-slide-to="'.($counter-1).'" class="'. esc_attr($active).'" ></li>';
        ...
        foreach ($post_attachments as $attachment) {
            ...
            $indicators.= ' <li data-target="#carousel-listing" data-slide-to="'.esc_attr($counter-1).'" class="'. $active.'">
                                <img  src="'.esc_url($preview[0]).'"  data-slider-no="'.esc_attr($counter).'" alt="'.esc_html__('image','wpresidence').'" />
                            </li>';
            $round_indicators   .=  ' <li data-target="#carousel-listing" data-slide-to="'.esc_attr($counter-1).'" class="'. esc_attr($active).'"></li>';

            $slides .= '<div class="item '.esc_attr($active).'">
                        <a href="'.esc_url($full_prty[0]).'" rel="prettyPhoto" class="prettygalery" title="'.esc_attr($attachment_meta['caption']).'" > 
                            <img  src="'.esc_url($full_img[0]).'"  data-slider-no="'.esc_attr($counter_lightbox).'" alt="'.esc_attr($attachment_meta['alt']).'"  class="img-responsive lightbox_trigger" />
                         </a>
                        </div>';
    ...
    if ( $header_type == 0 ){ // global
        ...
        if($header_type!=5){
                ...
                ?>
                <div id="slider_enable_slider" data-placement="bottom" data-original-title="<?php esc_attr__('Image Gallery','wpresidence');?>" class="slideron <?php echo   esc_attr($no_street); ?>"> <i class="fa fa-picture-o"></i>         </div>
