...
<div class="<?php echo esc_html($col_class);?> listing_wrapper property_unit_type3" data-org="<?php echo esc_attr($col_org);?>" data-listid="<?php echo intval($post->ID);?>" > 
    <div class="property_listing property_unit_type3 <?php if($wpestate_uset_unit==1) print 'property_listing_custom_design' ?> " data-link="<?php   if(  $wpestate_property_unit_slider==0){ echo esc_url($link);}?>">
            ...
            <div class="property_categ_unit_type3">
                <?php 
                    if($property_category!=''){
                        print '<span><strong>'.esc_html__('Category:','wpresidence').'</strong> '.wp_kses_post($property_category).', '.wp_kses_post($property_action_category).'</span>';
                    }
                ?>
            </div>
