...
<div id="gmap_wrapper"  class="<?php print esc_attr($gmap_class); echo wpresidence_return_class_leaflet(); ?>" <?php print trim($property_add_on); ?> style="height:<?php print intval($closed_height);?>px"  >
   ...
   <div id="gmap-loading"><?php esc_html_e('searching...','wpresidence');?> 
   ...
   <div class="gmap-controls <?php echo wpresidence_return_class_leaflet(); ?> ">
        ...
        if( isset($post->ID) && !is_tax() && !is_category()){
            if (wpestate_get_map_open_close_status($post->ID) == 0 ){
                print ' <div id="openmap"><i class="fa fa-angle-down"></i>'.esc_html__('open map','wpresidence').'</div>';
            }
        }else{
        ...
        ?>
        <div id="gmap-control">
            <span  id="map-view"><i class="fa fa-picture-o"></i><?php esc_html_e('View','wpresidence');?></span>
            ...
            <span  id="geolocation-button"><i class="fa fa-map-marker"></i><?php esc_html_e('My Location','wpresidence');?></span>
            <span  id="gmap-full" ><i class="fa fa-arrows-alt"></i><?php esc_html_e('Fullscreen','wpresidence');?></span>
        <?php     
            if( !is_singular('estate_property') ){ ?>
                <span  id="gmap-prev"><i class="fa fa-chevron-left"></i><?php esc_html_e('Prev','wpresidence');?></span>
                <span  id="gmap-next" ><?php esc_html_e('Next','wpresidence');?><i class="fa fa-chevron-right"></i></span>
        ...
        <?php 
            if( is_singular('estate_property') ){
                if ( get_post_meta($post->ID, 'property_google_view', true) ==1){
                    ?>
                    <div id="street-view" class="<?php echo esc_html($street_view_class);echo wpresidence_return_class_leaflet(); ?>"><i class="fa fa-location-arrow"></i> <?php esc_html_e('Street View','wpresidence');?> </div>
                    <?php   
                } 
            }
        ?>
