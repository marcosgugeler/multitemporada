...
<div class="col-md-4 listing_wrapper">
    <div class="agent_unit" data-link="<?php print esc_attr($link);?>">
        ...
        <div class="">
            ...
            if ($agent_phone) {
                print '<div class="agent_detail"><i class="fa fa-phone"></i>'.esc_html($agent_phone ).'</div>';
            }
            if ($agent_mobile) {
                print '<div class="agent_detail"><i class="fa fa-mobile"></i>'.esc_html($agent_mobile).'</div>';
            }

            if ($agent_email) {
                print '<div class="agent_detail"><i class="fa fa-envelope-o"></i>'. esc_html($agent_email).'</div>';
            }
        ...
        <div class="agent_control_bar ">
                ...
                <a  data-original-title="<?php esc_attr_e('Edit Agent','wpresidence');?>"   class="dashboad-tooltip" href="<?php  print esc_url($edit_link);?>"><i class="fa fa-pencil-square-o editprop"></i></a>
                ...
                <?php
                    if( $post_status == 'publish' ){ 
                        print ' <span  data-original-title="'.esc_attr__('Disable Agent','wpresidence').'" class="dashboad-tooltip disable_listing disable_agent disabledx" data-postid="'.intval($post->ID).'" ><i class="fa fa-pause"></i></span>';
                    }else if($post_status=='disabled') {
                        print ' <span  data-original-title="'.esc_attr__('Enable Agent','wpresidence').'" class="dashboad-tooltip disable_listing disable_agent" data-postid="'.intval($post->ID).'" ><i class="fa fa-play"></i></span>';
                    }
                ?>               
