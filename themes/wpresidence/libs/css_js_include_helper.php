    ...
    function wpestate_load_google_map(){
        ...
        if($what_map==1){
            ...
            if (!wp_script_is( 'googlemap', 'enqueued' )) {
                ...
                if ( is_ssl() ) {
                    wp_enqueue_script('googlemap', 'https://maps-api-ssl.google.com/maps/api/js?v=3.35'.$libraries.'&amp;key='.esc_html(wpresidence_get_option('wp_estate_api_key', '') ),array('jquery'), '1.0', true);        
                }else{
                    wp_enqueue_script('googlemap', 'http://maps.googleapis.com/maps/api/js?v=3.35'.$libraries.'&amp;key='.esc_html(wpresidence_get_option('wp_estate_api_key', '') ),array('jquery'), '1.0', true);        
                }
