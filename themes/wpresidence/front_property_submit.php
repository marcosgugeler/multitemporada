...
foreach ($status_values_array as $key=>$value) {
    ...
    if (function_exists('icl_translate') ){
        $value_wpml = icl_translate('wpestate','wp_estate_property_status_front_'.esc_html($slug_status),$value );
    }
    $property_status.='<option value="'.esc_html($value).'"';
    $property_status.='>'.esc_html(stripslashes($value_wpml)).'</option>';
}
...
if( isset($_POST) && isset($_POST['action']) && $_POST['action']=='front_submit' && is_user_logged_in()) {
    ...
    if ( $paid_submission_status!='membership' || ( $paid_submission_status== 'membership' || wpestate_get_current_user_listings($parent_userID) > 0)  ){ // if user can submit
        ...
        foreach ($status_values_array as $key=>$value) {
            ...
            $property_status.='>'.esc_html($value_wpml).'</option>';
        }
