...
function wpestate_admin_notice() {
    ...
    if( !is_array($wpestate_notices) || 
        !isset($wpestate_notices['wp_estate_update_1605']) ||
        ( isset($wpestate_notices['wp_estate_update_1605']) && $wpestate_notices['wp_estate_update_1605']!='yes')  ){
        
        print '<div  id ="setting-error-wprentals-cache"  data-notice-type="wp_estate_update_1605"  data-dismissible="disable-done-notice-forever" class="wpestate_notices updated settings-error notice is-dismissible">
            <p>'.esc_html__( 'Attention! For WpResidence v1.60.5 or later Child Theme wpresidence-child/functions.php must be updated as detailed in this ','wpresidence').'<a href="http://help.wpresidence.net/article/wpresidence-v1-60-5-march-25/" target="_blank">'.esc_html__('help article','wpresidence').'</a></p>
        </div>';
    }
    
       if( !is_array($wpestate_notices) || 
        !isset($wpestate_notices['wp_estate_update_170']) ||
        ( isset($wpestate_notices['wp_estate_update_170']) && $wpestate_notices['wp_estate_update_170']!='yes')  ){
        
        print '<div  id ="setting-error-wprentals-cache"  data-notice-type="wp_estate_update_170"  data-dismissible="disable-done-notice-forever" class="wpestate_notices updated settings-error notice is-dismissible">
            <p>'.esc_html__( 'IMPORTANT to Elementor PRO users: For compatibility with Elementor PRO all theme widgets have been redone as Elementor PRO doesn\'t support blank spaces in widget names. For that reason if you update the WPResidence Elementor Add-on to 1.70, Elementor PRO will work BUT all pages created with Elementor and Residence Elementor widgets before this version will have to be redone. If you wish to use Elementor PRO the Elementor add-on plugin update to 1.70 is mandatory and all theme widgets have to be redone.','wpresidence').'</p>
        </div>';
    }
...
if( !function_exists('wpestate_check_license_function') ):
    function wpestate_check_license_function(){
        ...
        $data= array('license'=>trim($wpestate_license_key));
